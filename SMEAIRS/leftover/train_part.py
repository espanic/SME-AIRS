...
from mymodel import MyModel_V4
...
def train(args):
    ...
    if args.train_cascade > 0 or args.retrain:
        ...
        model = MyModel_V4(num_airs_layers=args.train_cascade + 1,
                           sme_stateDict=modelDict['sme'],
    ...
    args.sme_inchans = model.sme_inchans

    f = os.path.join(args.config_dir, 'config_{}_fixed.txt'.format(args.train_cascade))
...
def validate(args, model, data_loader):
    ...
    with torch.no_grad():
        for i, data in enumerate(data_loader):
            input, target, mask, acs, maximum, _, ifnames, slices, _ = data

            input = input.cuda(non_blocking=True)
            acs = acs.cuda(non_blocking=True)

            output = model(input, acs)

            for i in range(output.shape[0]):
                reconstructions[ifnames[i]][int(slices[i])] = output[i].cpu().numpy()
                targets[ifnames[i]][int(slices[i])] = target[i].numpy()
                a = ifftc_torch(input[i])
                inputs[ifnames[i]][int(slices[i])] = rss_combine_torch(a, 0).cpu().numpy()
...
def train_epoch(args, model, optimizer, loss_type, epoch, data_loader):
    ...
    for i, data in enumerate(data_loader):
        input, target, _, acs, maximum, _, _, _ = data
        input = input.cuda(non_blocking=True)
