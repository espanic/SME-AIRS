...
from mymodel5 import MyModel_V9
...
def train_model9(args):
    device = torch.device(f'cuda:{args.GPU_NUM}' if torch.cuda.is_available() else 'cpu')
    torch.cuda.set_device(device)
    print('Current cuda device: ', torch.cuda.current_device())
    torch.cuda.empty_cache()

    train_loader = create_data_loader_modelv9(args.data_path_train, args, cropInput=True, shuffle=True)
    val_loader = create_data_loader_modelv9(args.data_path_val, args, cropInput=True, shuffle=False)
    ...
    if args.validate_only:
        ...
        model = MyModel_V9(airs_inchans=4)
        model.to(device=device)
        model.load_state_dict(dicts, load_weak_only=True)
    ...
    # train full network
    if not args.train_weak_only:
        state_dict = torch.load(args.weak_dir)['model']
        model.load_state_dict(state_dict, load_weak_only=True)
...
def validate(args, model, data_loader, loss_type):
    ...
    with torch.no_grad():
        for i, data in tqdm(enumerate(data_loader), desc="validation precedure", mininterval=0.01, total=length):
            input, grappa, target, mask, _, maximum, _, _, _ = data
            input = input.cuda(non_blocking=True)
            grappa = grappa.cuda(non_blocking=True)
            output = model(input, grappa, args.train_weak_only)
...
def train_epoch(args, model, optimizer, loss_type, epoch, data_loader):
    ...
    for i, data in tqdm(enumerate(data_loader), desc="train procedure ", mininterval=0.01, total=len(data_loader)):
        ...
        input = input.cuda(non_blocking=True)
        grappa = grappa.cuda(non_blocking=True)
        output = model(input, grappa, train_only_weak=args.train_weak_only)
        target = target.cuda(non_blocking=True)
        maximum = maximum.cuda(non_blocking=True)
...
def save_model(args, exp_dir, epoch, optimizer, best_val_loss, is_new_best, model):
    ...
    modelDict['weak_layer'] = model.weak_layer.state_dict()
    torch.save(
        {
            'epoch': epoch,
            'args': args,
