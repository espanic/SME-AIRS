...
def train_grappa_with_filter(args):
    ...
    if args.validate_only:
        ...
        model = MyModel_V6(num_airs_layers=args.train_cascade + 1,
    ...
    if args.train_cascade > 0 or args.retrain:
        ...
        model = MyModel_V6(airs_inchans=4, filtering=False)
    model.to(device=device)
    ...
    for epoch in range(startEpoch, args.num_epochs):
        model, optimizer, train_loss, time = train_epoch_both(args, model, optimizer, loss_type, epoch, train_loader)
...
def validate_both(args, model, data_loader, loss_type):
    model.eval()
    inputs = defaultdict(dict)
    reconstructions = defaultdict(dict)
    targets = defaultdict(dict)
...
def train_epoch(args, model, optimizer, loss_type, epoch, data_loader):
    ...
    return model, optimizer, total_loss, start - time.perf_counter()


def train_epoch_both(args, model, optimizer, loss_type, epoch, data_loader):
    total_loss = 0
    start_iter = time.perf_counter()
    start = start_iter
    print(f'Epoch #{epoch:2d} ............... {args.net_name} : {args.train_cascade} ...............')
