...
class AIRSLayer(torch.nn.Module):
    ...
    def __init__(self, in_chans: int = 2, out_chans: int = 2, num_pools: int = 4, chans: int = 32,
        ...
        for i in range(num_pools):
            if i == 0:
                self.down_sample_layers.append(FLMDM(in_chans, ch, first_layer=first_layer_optimize, kernel_size=3))
                self.down_sample_layers.append(FLMDM(ch, ch, kernel_size=3))
            else:
        ...
        self.same_sample_layers1 = FLMDM(ch, ch)
        self.same_sample_layers2 = FLMDM(ch, ch)
    ...
    def forward(self, x):
        ...
        x = self.same_sample_layers1(x)
        x = self.same_sample_layers2(x)
...
class FLMDM(torch.nn.Module):
    ...
    def __init__(self, in_chans: int = 1, out_chans: int = 1, sampling_method: str = 'same', kernel_size: int = 3,
        ...
        if sampling_method == 'up':
            ...
            if self.first_layer:
                self.conv_image = torch.nn.Conv2d(2, out_chans, kernel_size, padding=padding, bias=False)
            else:
    ...
    def forward(self, input):
        # image domain
        if self.first_layer:
            input1 = self.complex_to_chan_dim(input[..., :1])
