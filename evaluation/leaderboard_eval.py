    ...
    with torch.no_grad():
        for i_subject in range(58):
            ...
            for i_slice in range(num_slices):
                ...
                ssim = ssim_calculator(recon * mask, target * mask, maximum).cpu().numpy()
                ssims.append(ssim)
                ssim_total += ssim
                idx += 1
                # if ssim > 0.995:
                #     plot_image((recon).cpu().numpy())
                #     plot_image((target).cpu().numpy())
...
if __name__ == '__main__':
    ...
    """
    parser.add_argument('-yp', '--your_data_path', type=str, default='../result/SMEAIRS_v6_input_grappa/reconstructions_forward_level0/')
